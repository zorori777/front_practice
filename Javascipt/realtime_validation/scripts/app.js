...
AppModel.prototype.trigger = function(event) {
  // $はƒ (a,b){return new n.fn.init(a,b)}
...
AppModel.prototype.validate = function() {
  ...
  // this.attrsはメソッドを呼んでいる
  // {required: true, maxlength: 8, minlength: 4}
  for (var key in this.attrs) {
    val = this.attrs[key];
    // !this[key](val)はメソッドの返り値がfalseを期待する
    // valいるかな？
...
AppView.prototype.initialize = function(el) {
  ...
  // ulの子要素であるliを取り出し、それに紐付いているdata属性を取り出している
  var obj = this.$el.data();

  // 一括削除をしてformを空にした時などに使う
